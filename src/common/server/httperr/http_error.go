...
import (
	...
	"github.com/jeanmolossi/literate-robot/src/common/errors"
...
func httpRespondWithError(err error, slug string, w http.ResponseWriter, r *http.Request, logMessage string, statusCode int) {
	logs.GetLogEntry(r).WithError(err).WithField("error-slug", slug).Warn(logMessage)

	resp := ErrorResponse{slug, statusCode}
